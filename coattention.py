...
class CoAttention(nn.Module):
    ...
    def __init__(
        ...
        self.map_comments = HypAct(
            manifold, self.comment_curvature, self.c, act=torch.tanh
        )
        self.map_content = HypAct(
            manifold, self.content_curvature, self.c, act=torch.tanh
        )
    ...
    def forward(self, sentence_rep, comment_rep):
        ...
        Hs_a = mobius_matvec(
            self.Ws, sentence_rep_trans.transpose(-1, -2), c=curv
        ).transpose(-1, -2)
        Hs_a = proj(Hs_a, c=curv)
        Hs_b = mobius_matvec(
            self.Wc, comment_rep_trans.transpose(-1, -2), c=curv
        ).transpose(-1, -2)
        ...
        Hs = proj(
            expmap0(torch.tanh(logmap0(Hs, c=curv)), c=curv), c=curv
        )  # [32, 80, 50]
        ...
        As = mobius_matvec(self.whs, Hs.transpose(-1, -2), c=curv).transpose(
            -1, -2
        )  # [32, 1, 50]
        As = proj(As, c=curv)
