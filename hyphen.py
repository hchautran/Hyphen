...
class Hyphen(nn.Module):
    ...
    def forward(self, content, comment, subgraphs):
        ...
        #only comment module is on
            ...
            comment_embedding = self.comment_encoder(comment, comment.ndata['feat'], subgraphs)
            comment_embedding = self.manifold.proj(comment_embedding, k = 1.0)
            comment_embedding = self.manifold.logmap0(comment_embedding, k = 1.0)
        ...
        #only content module is on
            ...
            content_embedding = self.manifold.proj(content_embedding, k = 1.0)
            content_embedding = self.manifold.logmap0(content_embedding, k = 1.0)
