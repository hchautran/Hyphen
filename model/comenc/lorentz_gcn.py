...
class ComEnc(nn.Module):
    ...
    def forward(self, g, h, subgraphs):
        ...
        if isinstance(self.manifold, PoincareBall):
            inp = torch.cat(
                [
                    self.manifold.expmap0(i).unsqueeze(0)
                    for i in inp
                ],
                axis=0,
            )
        ...
        h = self.manifold.logmap0(self.manifold.projx(h))
        ...
        if self.content_module:
            with g.local_scope():
                ...
                for batch_idx in range(len(unbatched)):
                    ...
                    agg = self.manifold.projx(
                        self.manifold.expmap0(agg)
                    )
            ...
            with g.local_scope():
                ...
                ret = self.manifold.projx(self.manifold.expmap0(ret))
                return ret
