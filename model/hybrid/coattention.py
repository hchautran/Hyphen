...
class CoAttention(nn.Module):
    ...
    def __init__(
        ...
        self.manifold = PoincareBall(curv) 
        ...
        self.map_comments = HypAct(
            self.manifold, self.comment_curvature, self.c, act=torch.tanh
        )
        self.map_content = HypAct(
            self.manifold, self.content_curvature, self.c, act=torch.tanh
        )
    ...
    def forward(self, sentence_rep, comment_rep):
        ...
        if self.fourier:
            ...
            sentence_rep = proj(expmap0(sentence_rep))
            comment_rep = proj(expmap0(comment_rep))
