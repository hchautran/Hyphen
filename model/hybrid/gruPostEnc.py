...
class GRUPostEnc(nn.Module):
    def __init__(self, word_hidden_size, sent_hidden_size, batch_size, num_classes, embedding_matrix, curv):
        super(GRUPostEnc, self).__init__()
        self.batch_size = batch_size
        self.word_hidden_size = word_hidden_size
        self.sent_hidden_size = sent_hidden_size
        self.manifold = PoincareBall(c=curv) 
        self.curv= self.manifold.c 
        self.word_att_net = H_WordAttNet(embedding_matrix, word_hidden_size, curv=self.curv)
        self.sent_att_net = H_SentAttNet(sent_hidden_size, word_hidden_size, num_classes, self.curv)
...
class H_WordAttNet(nn.Module):
    def __init__(self, embedding_matrix, hidden_size = 50, curv=1.0):
        super().__init__()
        # for dot attention
...
class H_SentAttNet(nn.Module):
    def __init__(self, sent_hidden_size=50, word_hidden_size=50, num_classes=2, curv = 1):
        super().__init__()
        ...
        self.dot_att_us = MobiusLinear(2*sent_hidden_size, 1, bias=False, c=self.c)
        self.hyp_att_projector = MobiusLinear(2*sent_hidden_size, 2*sent_hidden_size, bias=True, c=self.c) #Todo{discard}
        self.dot_att_projector = MobiusLinear(2*sent_hidden_size, 2*sent_hidden_size, bias=True, c=self.c)
        self.logit_projector = MobiusLinear(2*sent_hidden_size, sent_hidden_size, bias=True, c=self.c)
