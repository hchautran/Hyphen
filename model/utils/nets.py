    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.bias is not None:
            if hyperbolic_bias:
                self.ball = manifold = geoopt.PoincareBall()
                self.bias = geoopt.ManifoldParameter(self.bias, manifold=manifold)
                with torch.no_grad():
                    self.bias.set_(pmath.expmap0(self.bias.normal_() / 4, k=self.c))
        else:
            self.ball = manifold = geoopt.PoincareBall()
        with torch.no_grad():
...
class MobiusDist2Hyperplane(torch.nn.Module):
    def __init__(self, in_features, out_features, c=1.0):
        ...
        self.ball = ball = geoopt.PoincareBall()
        ...
        point = pmath.expmap0(point, k=torch.tensor(c))
    ...
    def forward(self, input):
        ...
        distance = pmath.dist2plane(
            x=input, p=self.point, a=self.tangent, k=self.k, signed=True
        )
