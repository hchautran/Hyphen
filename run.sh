...
do
    ...
    do 
        for dataset in antivax pheme figlang_twitter 
        do
            CUDA_VISIBLE_DEVICES=$1 python run.py --manifold $2 --lr 0.001 --dataset $dataset --batch-size 30 --epochs 100 --max-sents 30 --max-coms 30 --max-com-len 20 --max-sent-len 20 --log-path logging/run --model $model --embedding-dim $embedding_dim 
